...
//сохранялка
    ...
    public MainFileChooser(MainFrame frame) {
        ...
        //выбираем для показа расширение файла
        setAcceptAllFileFilterUsed(false);//запрещает пользователю указать другой тип файлов
        setCurrentDirectory(Settings.SAVE_DIR);//по умолчанию для сохранения будет открывать нужную нам директорию
