...
public class TransferTableModel extends MainTableModel {
    ...
    public TransferTableModel(String[] columns) {
        super(SaveData.getInstance().getFilterTransfers(), columns);//нужны переводы по фильтру (дате)
    }

    @Override
    protected void updateData() {
        data = SaveData.getInstance().getFilterTransfers();// если уже что-то есть, значит используем последние транзакции
    }
