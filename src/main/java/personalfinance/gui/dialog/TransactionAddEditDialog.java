...
public class TransactionAddEditDialog extends AddEditDialog {
    ...
    protected void init() {
        ...
        values.put("LABEL_AMOUNT", Format.amount(0));//формат по умолчанию 0
    }

    @Override
    protected void setValues() { // устанавливаем значения
        Transaction transaction = (Transaction) c;// приводим "Common c"  к Account
    ...
    public Common getCommonFromForm() throws ModelException { // обрабатываем
        try {
            ...
            return new Transaction(account, article, Format.fromAmountToNumber(amount), notice, date);// если вдруг введут вместо цифр буквы, для этого try/catch
